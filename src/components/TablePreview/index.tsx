  ...
  const memoizedTransactions = useMemo(() => {
    return rows.map((row: IRow, rowIndex: number) => {
      ...
      return (
        ...
        >
          <td
            className={`sticky left-0 border-r-[1px] border-b-zinc-300 sm:border-0 ${row.isToday ? 'bg-grayWhite dark:bg-zinc-800' : 'bg-white'} dark:bg-black-bg dark:text-softGray sm:relative border-b-1 p-2 border-lineGray text-sm text-center font-[600] h-[56px]`}
          >
            {row.formatted_date}
          </td>
          <td
